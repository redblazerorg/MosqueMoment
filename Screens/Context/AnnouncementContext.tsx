...
interface AnnouncementContextType {
  ...
  // admin functions
  addMosque: (mosqueName: string, description: string, picture: string) => Promise<void>;
  ...
  updateMosque: (mosqueId: number, updates: Partial<MosqueDetails>) => Promise<MosqueDetails | undefined>;
  addAnnouncement: (mosqueId: number, announcement: Omit<Announcement, 'id' | 'createdAt'>) => Promise<void>;
  deleteAnnouncement: (mosqueId: number, announcementId: number) => Promise<void>;
  addActivity: (mosqueId: number, activity: Omit<Activity, 'id' | 'createdAt'>) => Promise<void>;
...
export const AnnouncementProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const [filteredMosqueDetails, setFilteredMosqueDetails] = useState<MosqueDetails[]>([]);
  ...
  const loadMosqueDetails = async () => {
    try {
      ...
      if (storedMosques) {
        ...
        const mosquesWithoutImages = parsedMosques.map((mosque: MosqueDetails) => {
  ...
  const addMosque = async (mosqueName: string, description: string, picture: string) => {
    if (!user?.email || user.role !== 'admin') return;

    // Check if admin already manages a mosque
    const existingMosque = mosqueDetails.find(mosque => mosque.adminId === user.email);
    if (existingMosque) {
  ...
  const getMosqueByAdminId = (adminEmail: string): MosqueDetails | undefined => {
  ...
  const updateMosque = async (mosqueId: number, updates: Partial<MosqueDetails>) => {
    if (!user?.email || user.role !== 'admin') return;
  ...
  const addAnnouncement = async (mosqueId: number, announcement: Omit<Announcement, 'id' | 'createdAt'>) => {
    const updatedMosques = mosqueDetails.map(mosque => {
      if (mosque.id === mosqueId && mosque.adminId === user?.email) {
  ...
  const deleteAnnouncement = async (mosqueId: number, announcementId: number) => {
    const updatedMosques = mosqueDetails.map(mosque => {
      if (mosque.id === mosqueId && mosque.adminId === user?.email) {
        return {
          ...mosque,
          announcement: mosque.announcement.filter(a => a.id !== announcementId),
        };
      }
      return mosque;
    });
  ...
  const addActivity = async (mosqueId: number, activity: Omit<Activity, 'id' | 'createdAt'>) => {
  ...
  const subscribe = async (id: number) => {
    ...
    const updatedMosques = mosqueDetails.map(mosque =>
      mosque.id === id ? { ...mosque, isSubscribe: !mosque.isSubscribe } : mosque
    );
    await saveMosqueDetails(updatedMosques);
    ...
    if (user?.email) {
      try {
        const subscriptionsJson = await AsyncStorage.getItem(`subscriptions_${user.email}`);
        let subscriptions = subscriptionsJson ? JSON.parse(subscriptionsJson) : [];
        ...
        await AsyncStorage.setItem(`subscriptions_${user.email}`, JSON.stringify(subscriptions));
      } catch (error) {
  ...
  const searchMosque = (mosqueName: string, type: 'mosque' | 'activity' = 'mosque') => {
    if (mosqueName.trim() === "") {
